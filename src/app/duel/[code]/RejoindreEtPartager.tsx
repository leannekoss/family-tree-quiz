...
export default function RejoindreEtPartager({
  duelId,
  code,
  dejaMembre,
}: {
  ...
  const [rejoint, setRejoint] = useState(dejaMembre);
  ...
  async function rejoindre() {
    ...
    await supabase
      .from("duel_members")
      .upsert({ duel_id: duelId, user_id: user.id });
    setRejoint(true);
    router.refresh();
  }

  const lien = `${SITE}/duel/${code}`;
  const texte = `Je te défie au quiz de la famille ! Qui connaît le mieux les cousins ? ${lien}`;
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="inline-flex items-center gap-2 rounded-lg bg-[#128C7E] px-5 py-2.5 text-sm font-medium text-white"
      >
      ...
      <button
        type="button"
        onClick={partager}
        className="inline-flex items-center px-2 text-sm text-muted underline underline-offset-4"
      >
